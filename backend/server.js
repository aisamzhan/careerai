...
const { sendMail, hasSmtpConfig } = require('./auth/mailer');
...
function randomToken(bytes = 32) {
  // Base64url token suitable for URLs (no + / = characters).
  return crypto
...
function getAppBaseUrl() {
  const v = String(process.env.APP_BASE_URL || '').trim();
  return v || 'http://localhost:5173';
}

function emailVerificationEnabled() {
  const flag = String(process.env.EMAIL_VERIFICATION_ENABLED || '1').trim();
  return flag !== '0' && hasSmtpConfig();
}
...
app.post('/api/auth/register', async (req, res) => {
  ...
  try {
    ...
    // If SMTP is configured, require verification for new accounts.
    const requireVerify = emailVerificationEnabled();
    ...
    if (requireVerify) {
      ...
      await setEmailVerificationToken({ userId: user.id, token, ttlHours: 24 });

      const appBase = getAppBaseUrl();
      const verifyUrl = `${appBase.replace(/\/$/, '')}/verify-email?token=${encodeURIComponent(token)}`;
      await sendMail({
        to: user.email,
        subject: 'Verify your email for CareerAI',
        text: `Welcome to CareerAI!\n\nPlease verify your email by opening this link:\n${verifyUrl}\n\nThis link expires in 24 hours.`,
        html: `<p>Welcome to <b>CareerAI</b>!</p>
...
<p>This link expires in 24 hours.</p>`,
    ...
    // No SMTP configured: keep old behavior so signups still work.
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    if (emailVerificationEnabled() && user.email_verified === false) {
...
app.post('/api/auth/verify/request', async (req, res) => {
  ...
  try {
    const user = await findUserByEmail(email);
    if (!user) return res.json({ ok: true, enabled: true });
    if (user.email_verified) return res.json({ ok: true, enabled: true });
    ...
    await sendMail({
      ...
      html: `<p>Verify your email by opening this link:</p><p><a href="${verifyUrl}">${verifyUrl}</a></p><p>This link expires in 24 hours.</p>`,
    });
...
app.post('/api/auth/verify/confirm', async (req, res) => {
  ...
  if (!token) return res.status(400).json({ error: 'Missing field: token' });

  if (!emailVerificationEnabled()) {
    return res.status(400).json({ error: 'Email verification is not enabled on this server' });
  }

  try {
    const user = await verifyEmailByToken({ token });
    if (!user) return res.status(400).json({ error: 'Invalid or expired token' });
    return res.json({ ok: true, user: { id: user.id, email: user.email, emailVerified: user.email_verified } });
  } catch (err) {
    res.status(500).json({ error: String(err && err.message ? err.message : err) });
  }
});
...
app.post('/api/auth/password-reset/request', async (req, res) => {
  ...
  // Return ok even if user does not exist (avoid enumeration).
  try {
    const user = await findUserByEmail(email);
    if (!user) return res.json({ ok: true });

    const token = randomToken(32);
    await setPasswordResetTokenByEmail({ email, token, ttlMinutes: 30 });

    const appBase = getAppBaseUrl();
    const resetUrl = `${appBase.replace(/\/$/, '')}/reset-password?token=${encodeURIComponent(token)}`;
    await sendMail({
      to: email,
      subject: 'Reset your CareerAI password',
      text: `To reset your password, open this link:\n${resetUrl}\n\nThis link expires in 30 minutes. If you did not request this, you can ignore this email.`,
      html: `<p>To reset your password, open this link:</p><p><a href="${resetUrl}">${resetUrl}</a></p><p>This link expires in 30 minutes.</p><p>If you did not request this, you can ignore this email.</p>`,
    });
...
app.post('/api/auth/password-reset/confirm', async (req, res) => {
  ...
  if (!token) return res.status(400).json({ error: 'Missing field: token' });
  if (!newPassword) return res.status(400).json({ error: 'Missing field: newPassword' });

  try {
    const newHash = hashPassword(newPassword);
    const user = await consumePasswordResetToken({ token, newPasswordHash: newHash });
    if (!user) return res.status(400).json({ error: 'Invalid or expired token' });
    return res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: String(err && err.message ? err.message : err) });
